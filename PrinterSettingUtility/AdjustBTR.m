...
@implementation AdjustBTRController
...
- (id)init
{
    ...
    if (self) {
       ...
       contentTitle= NSLocalizedString(@"Adjust BTR", NULL);
        SettingsName = NSLocalizedString(@"Adjust BTR", NULL);
...
{
    return NSLocalizedString(@"Adjust BTR", NULL);
}
...
{
    DEV_ADJUST_BTR settings;
    memset(&settings, 0, sizeof(DEV_ADJUST_BTR));
    ...
    memcpy(deviceData, &settings, sizeof(DEV_ADJUST_BTR));
...
- (IBAction)onApplyNewSettings:(id)sender {
    ...
    for(i = 0; i < [devciePropertyList count]; i++)
    {
        ...
        [aCommond setDeviceData:(void*)&settings dataSize:sizeof(DEV_ADJUST_BTR)];
    }
