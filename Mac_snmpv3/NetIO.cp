...
bool CNetIO::FindSnmpAgent(const char* community, const char* ip, const char* oid, LPFNFINDCALLBACK FindCallback, void* param, bool bBroadcast)
{
	...
	const char request_oid_pf[] = {0xa0,0x19,0x02,0x01,0x00,0x02,0x01,0x00,0x02,0x01,0x00,
...
bool CNetIO::NetworkReadStatusV6(const char* community, const char* ip, PRINTER_STATUS *status)
{
	bool bRet = false;
	
	char std_liteon_device_id_req[51] = {0x30, 0x31, 0x02, 0x01, 0x00, 0x04, 0x06, 0x70, 0x75, 0x62, 0x6c, 0x69, 0x63, 0xa0, 0x24, 0x02, 0x01, 0x00, 0x02, 0x01, 0x00, 0x02, 0x01, 0x00, 0x30, 0x19, 0x30, 0x17, 0x06, 0x13, 0x2b, 0x06, 0x01, 0x04, 0x01, 0x81, 0xcd, 0x1a, 0x86, 0x76, 0x82, 0x2c, 0x82, 0x71, 0xc2, 0x53, 0x01, 0x01, 0x00, 0x05, 0x00, };
...
bool CNetIO::NetworkReadStatusV6SNMPv3(const char* community, const char* ip, PRINTER_STATUS *status)
{
	bool bRet = false;
	int nSendSock;
	int nRet;
...
bool CNetIO::NetworkReadStatus(const char* community, const char* ip, PRINTER_STATUS *status)
{
	bool bRet = false;
	...
	if(result->ai_family == AF_INET6)
		return NetworkReadStatusV6(community,ip,status);
...
bool CNetIO::NetworkReadStatusSNMPv3(const char* community, const char* ip, PRINTER_STATUS *status)
{
	bool bRet = false;
	struct addrinfo *result;
	/* resolve the domain name into a list of addresses */
	...
	if(result->ai_family == AF_INET6)
		return NetworkReadStatusV6SNMPv3(community,ip,status);
